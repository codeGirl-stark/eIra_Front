...
export default function Step2({prevStep, formData, handleChange,handleCheckboxChange,nextStep }) {
    const [metastaseOptions, setMetastaseOptions] = useState([]);

    // Gestion du changement pour le champ "Autres de metastase"
    const MetastaseHandleOtherTextChange = (e) => {
        const value = e.target.value;
        setMetastaseOptions(value);
        ...
        if (!formData.circonstance.includes("Autres")) {
            handleCheckboxChange("metastase", "Autres", true);
        }

        // // Ajouter dynamiquement "Autre" au tableau metastase
        // if (!formData.metastase.includes(value)) {
        //     handleCheckboxChange("metastase", value, true);
        // }
    };

    const MetastasehandleOtherCheckbox = (isChecked) => {
        if (!isChecked) {
            // Si décoché, retirer "Autre" et son texte
            setMetastaseOptions("");
            handleCheckboxChange("metastase", "Autres", false);
        } else {
            handleCheckboxChange("metastase", "Autres", true);
        }
    };
                    ...
                    <div className="rounded-lg border mb-10 border-gray-500 bg-white shadow-lg p-6 dark:border-gray-600 dark:bg-blue-950">
                            ...
                            <div>
                                ...
                                <div className="space-y-2">
                                    {METASTASE_OPTIONS.map((option) => (
                                        <div key={option.value} className="flex items-center">
                                            <input
                                                ...
                                                id={option.value}
                                                value={option.value}
                                                checked={formData.metastase.includes(option.value)}
                                                onChange={(e) => {
                                                    if (option.value === "Autres") {
                                                        MetastasehandleOtherCheckbox(e.target.checked);
                                                    } else {
                                    ...
                                    {formData.metastase.includes("Autres") && (
                                        <div className="mt-4">
                                            <label
                                            ...
                                            <input
                                                ...
                                                onChange={MetastaseHandleOtherTextChange}
                                    ...
                                    {CURAGE_OPTIONS.map((option) => (
                                        <div key={option.value} className="flex items-center">
                                            <input
                                                ...
                                                id={option.value}
                                                value={option.value}
                                                checked={formData.curage.includes(option.value)}
                                                onChange={(e) =>
                                                    handleCheckboxChange(
                                                        "curage",
                                                        option.value,
