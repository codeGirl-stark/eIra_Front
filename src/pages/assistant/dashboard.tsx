...
export const Dashboard: React.FC = () =>  {
  ...
  useEffect(() => {
    const fetchPatient = async () => {
        ...
        await axios.get(`${apiUrl}/assistant/statistiques/`, {
            headers: {
    ...
    const fetchUser = async () => {
      ...
      .then(response =>{
          if (response.status === 200) {
              const mot = response.data.username.replace(/^as/, "");
              setNom(mot)
          } else {
        ...
        {statistiques && statistiques.total_patients.count > 0  ?(
          <>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
              <CardDataStats title="Patients du docteur" total={statistiques.total_patients.count.toString()} rate={`${statistiques.total_patients.percentage} %`}>
                <svg 
              ...
              <CardDataStats title="Dossiers du docteur" total={statistiques.total_dossiers.count.toString()} rate={`${statistiques.total_dossiers.percentage} %`} >
                <svg 
                  className="fill-primary dark:fill-white"
                  xmlns="http://www.w3.org/2000/svg" 
                  height="24px" 
