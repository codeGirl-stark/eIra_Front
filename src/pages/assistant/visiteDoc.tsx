...
const VisitesByDate = () => {
    ...
    useEffect(() => {
        const fetchPseudo = async () => {
            ...
            await axios.get(`${apiUrl}/assistant/getPseudoDoc/`, {
                headers: {
                ...
                .then(response =>{
                    setMedecin(response.data.username); // Enregistre les Patients dans le state
                })
                .catch(error =>{
    ...
    useEffect(() => {
        const fetchAllVisites = async () => {
            const access = localStorage.getItem('access_token');
            if (!access) {
                router.push("/assitant/login");
                return;
            }
    
            await axios.get(`${apiUrl}/assistant/rendezvous/`, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    'Authorization': `Bearer ${access}`,
                }
  ...
  return loading ? (
    ...
    <DefaultLayout>
        <Breadcrumb pageName ={`Les rendez-vous du docteur ${medecin.replace(/^dr/, "")}`} />
                    ...
                    <thead>
                      ...
                      {currentPatients.map((visite: VisiteInterface, index:number) => (
                          <tr key={visite.id}>
                              ...
                              <td className="border-b border-[#eee] dark:border-gray-500 py-5 px-5 xl:pl-11 dark:text-white ">
                                  <div className="flex items-center space-x-3">
                                      <Link href={`/assistant/Visite/${visite.id}`}>
