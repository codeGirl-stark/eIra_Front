...
export const ListeAdmin : React.FC = () => {
    ...
    useEffect(() => {
        const fetchAdmin = async () => {
            ...
            await axios.get(`${apiUrl}/admin_app/register_admins/`, {
                headers: {
    ...
    const currentMedecins = admins.slice(indexOfFirstMedecin, indexOfLastMedecin);

    const toggleAssistant = async (id: number, currentStatus: boolean) => {
        if (window.confirm(`Voulez-vous vraiment ${currentStatus ? "désactiver" : "activer"} cet administrateur ?`)) {
            setLoading(true);
            ...
            await axios.patch(`${apiUrl}/admin_app/register_admins/${id}/`,{'is_active': !currentStatus}, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                ...
                .then(() =>{
                    alert(`Administrateur ${currentStatus ? "désactivé" : "activé"} avec succès !`);
                    setAdmins((prev) =>
                        prev.map((doc) =>
    ...
    const handleDelete = async (id : any) => {
        if (window.confirm("Êtes-vous sûr de vouloir supprimer cet administrateur ?")) {
            ...
            await axios.delete(`${apiUrl}/admin_app/register_admins/${id}/`, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    'Authorization': `Bearer ${accessToken}`,
                },
                ...
                .then(() =>{
                    ...
                    setAdmins((prevMedecin) => prevMedecin.filter(medecin => medecin.id !== id));
                    setLoading(false)
                })
                .catch(error =>{
    ...
    const totalPages = Math.ceil(admins.length / medecinsPerPage);
    ...
    return loading ? (
        ...
        <DefaultLayout>
            ...
            <div className="flex justify-end my-5">
                <Link href="newAdmin" className="dark:text-white flex ">
                    <svg 
            ...
            {admins.length == 0 ? (
            <p className="dark:text-white text-xl">Aucun Administrateur Enregistré.</p>
            ) :(
                <div 
                ...
                >                
                    <div className="max-w-full overflow-x-auto">
                            <table className="w-full table-auto">
                                ...
                                <tbody>
                                {currentMedecins.map((admin: AdminInterface, index:number) => (
                                    <tr key={admin.id}>
                                        ...
                                        <td className="border-b border-[#eee] dark:border-gray-500 py-5 px-5 dark:text-white ">
                                            <div className="flex items-center space-x-3">
                                                <Link href={`/admin/Administrateurs/${admin.id}`}>
                                                ...
                                                <label className="flex items-center cursor-pointer">
                                                    <input
                                                        ...
                                                        onChange={() => toggleAssistant(admin.id, admin.is_active)}
                                                    />
