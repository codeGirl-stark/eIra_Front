...
export const AllAssistants: React.FC = () => {
    const router = useRouter();
    const apiUrl = process.env.NEXT_PUBLIC_API_URL;
    const [loading, setLoading] = useState<boolean>(true);
    const [medecins, setMedecins] = useState<AssistantInterface[]>([]);
    ...
    useEffect(() => {
        const fetchInstitutions = async () => {
            ...
            await axios.get(`${apiUrl}/admin_app/institutions/`, {
                headers: {
                ...
                .catch(error =>{
                    alert(error?.response?.data?.erreur || "Erreur lors de la récupération des institutions!");
                    console.log(error)
                })
        }
    ...
    const toggleAssistant = async (id: number, currentStatus: boolean) => {
        if (window.confirm(`Voulez-vous vraiment ${currentStatus ? "désactiver" : "activer"} cette institution ?`)) {
            setLoading(true);
            ...
            await axios.patch(`${apiUrl}/admin_app/institutions/${id}/`,{'is_active': !currentStatus}, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                ...
                .then(() =>{
                    alert(`Institution ${currentStatus ? "désactivée" : "activée"} avec succès !`);
                    setMedecins((prev) =>
                        prev.map((doc) =>
    ...
    const handleDelete = async (id : any) => {
        if (window.confirm("Êtes-vous sûr de vouloir supprimer cette institution ?")) {
            ...
            await axios.delete(`${apiUrl}/admin_app/institutions/${id}/`, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    'Authorization': `Bearer ${accessToken}`,
                },
                ...
                .catch(error =>{
                    alert(error?.response?.data?.erreur || "Erreur lors de la suppression de l'institution!");
                    console.log(error)
                    setLoading(false)
                })
        }
    ...
    return loading ? (
        ...
        <DefaultLayout>
            <Breadcrumb pageName="Liste des institutions" />

            <div className="flex justify-end my-5">
                <Link href="newInstitu" className="dark:text-white flex ">
                    <svg 
            ...
            {medecins.length == 0 ? (
            <p className="dark:text-white text-xl">Aucune Institution Enregistrée.</p>
            ) :(
                <div 
                ...
                >                
                    <div className="max-w-full overflow-x-auto">
                            <table className="w-full table-auto">
                                ...
                                <tbody>
                                {currentMedecins.map((assistant: AssistantInterface, index:number) => (
                                    <tr key={assistant.id}>
                                        ...
                                        <td className="border-b border-[#eee] dark:border-gray-500 py-5 px-5 dark:text-white ">
                                            <div className="flex items-center space-x-3">
                                                <Link href={`/admin/Institution/${assistant .id}`}>
