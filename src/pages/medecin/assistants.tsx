...
export const ListeAssistants: React.FC = () => {
    const router = useRouter();
    const apiUrl = process.env.NEXT_PUBLIC_API_URL;
    const [loading, setLoading] = useState<boolean>(true);
    const [assistants, setAssistants] = useState<AssistantInterface[]>([]);
    const [currentPage, setCurrentPage] = useState<number>(1); // Page actuelle
    const [assistantsPerPage] = useState<number>(10); // Nombre de patients par page
    ...
    useEffect(() => {
        const fetchAssistant = async () => {
            ...
            await axios.get(`${apiUrl}/admin_app/assistants/`, {
                headers: {
                ...
                .then(response =>{
                    setAssistants(response.data); // Enregistre les Patients dans le state

                })
                .catch(error =>{
                    alert(error?.response?.data?.erreur || "Erreur lors de la récupération des assistants!");
                    console.log(error)
                })
        }
    ...
    const currentAssistants = assistants.slice(indexOfFirstAssistant, indexOfLastAssistant);


    const toggleAssistant = async (id: number, currentStatus: boolean) => {
        if (window.confirm(`Voulez-vous vraiment ${currentStatus ? "désactiver" : "activer"} cet assistant ?`)) {
            setLoading(true);
            ...
            await axios.patch(`${apiUrl}/admin_app/assistants/${id}/`,{'is_active': !currentStatus}, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                ...
                .then(() =>{
                    alert(`Assistant ${currentStatus ? "désactivé" : "activé"} avec succès !`);
                    setAssistants((prev) =>
                        prev.map((assistant) =>
                            assistant.id === id ? { ...assistant, is_active: !currentStatus } : assistant
                    ));
                    setLoading(false)
                })
                .catch(error =>{
    ...
    const handleDelete = async (id : any) => {
        if (window.confirm("Êtes-vous sûr de vouloir supprimer cet assistant ?")) {
            ...
            await axios.delete(`${apiUrl}/admin_app/assistants/${id}/`, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    'Authorization': `Bearer ${accessToken}`,
                },
                ...
                .then(() =>{
                    alert("Assistant supprimé avec succès !"); // Enregistre les Patients dans le state
    ...
    return loading ? (
        ...
        <DefaultLayout>
            <Breadcrumb pageName="Liste des Assistants" />

            <div className="flex justify-end my-5">
                <Link href="/medecin/newAssistant" className="dark:text-white flex ">
                    <svg 
            ...
            {assistants.length == 0 ? (
            <p className="dark:text-white text-xl">Aucun assistant enregistré.</p>
            ) :(
                <div 
                ...
                >                
                    <div className="max-w-full overflow-x-auto">
                            <table className="w-full table-auto">
                                ...
                                <tbody>
                                {currentAssistants.map((assistant: AssistantInterface, index:number) => (
                                    <tr key={assistant.id}>
                                        <td className="border-b border-[#eee] py-5 px-4 dark:border-gray-500 sm:text-sm">
                                            <p className="text-black dark:text-white font-bold">
                                                {indexOfFirstAssistant + index + 1}
                                            </p>
                                        </td>
                                        <td className="border-b border-[#eee] py-5 px-4 dark:border-gray-500 sm:text-sm">
                                        ...
                                        <td className="border-b border-[#eee] py-5 dark:border-gray-500 sm:text-sm">
                                            <p className="text-black dark:text-white">
                                                {assistant.email}
                                            </p>
                                        </td>
                                        <td className="border-b border-[#eee] py-5 px-4 dark:border-gray-500 lg:pl-[5%] sm:text-sm">
                                            <p className="text-black dark:text-white">
                                                {assistant.is_active? "oui":"non"}
                                            </p>
                                        </td>

                                        <td className="border-b border-[#eee] dark:border-gray-500 py-5 px-5 dark:text-white ">
                                            <div className="flex items-center space-x-3">
                                                <Link href={`/medecin/Assistant/${assistant.id}`}>
...
export default ListeAssistants;